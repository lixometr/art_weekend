...
const srcPath = (file, watch = false) => {
  ...
  if (file === "hbs") return "./src/**/*.hbs";
  if (file === "hbs-compile") return ["./src/**/*.hbs", "!**/partials/**"];
...
const distPath = (file, serve = false) => {
  ...
  if (file === "hbs" && serve === false) return "./dist/*.hbs";
  if (file === "hbs" && serve === true) return "./dist";
...
const genericTask = (mode, context = "building") => {
  ...
  const browserLoadingWatching = (done) => {
    ...
    // Watch - Markup
    gulp.watch(srcPath("hbs"), true).on(
      "all",
      gulp.series(
        Object.assign(cleanMarkup(mode), {
